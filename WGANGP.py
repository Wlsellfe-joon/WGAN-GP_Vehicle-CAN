...
class RandomWeightedAverage(_Merge):
    ...
    def _merge_function(self, inputs):
        alpha = K.random_uniform((self.batch_size, 1, 1, 1)) #랜덤 수치 가져옴
        return (alpha * inputs[0]) + ((1 - alpha) * inputs[1]) # 진짜 이미지(input[0])와 가짜 이미지(input[1]) 사이 보간 이미지를 알파에 따라 반환
...
class WGANGP():
    ...
    def gradient_penalty_loss(self, y_true, y_pred, interpolated_samples): # Loss Function 사용자 정의
        ''' Computes gradient penalty based on prediction and weighted real / fake samples '''

        gradients = K.gradients(y_pred, interpolated_samples)[0] # RandomWeightedAverage의 출력이 interpolated_sample임
                                                                 # 해당 보간 이미지에 대한 예측 y_pred사이 거리 반환
    ...
    def wasserstein(self, y_true, y_pred): # Wasserstein Loss
        return -K.mean(y_true * y_pred)

    def get_activation(self, activation): # Activation Function
    ...
    def _build_generator(self):
        ...
        for i in range(self.n_layers_generator):
            ...
            if i < self.n_layers_generator - 1:
                ...
                x = Activation('tanh')(x) # 생성자 마지막 층은 tanh 사용
    ...
    def _build_adversarial(self):
        ...
        partial_gp_loss = partial(self.gradient_penalty_loss, #기존 함수에 들어가는 인자를 보간 이미지로 정의하여 새로운 함수 정의
                                  interpolated_samples=interpolated_img)
        partial_gp_loss.__name__ = 'gradient_penalty'  # Keras requires function names

        self.critic_model = Model(inputs=[real_img, z_disc], # 노이즈를 입력받는게 아니라 Shape을 맞춰주는 것
                                  outputs=[valid, fake, validity_interpolated]) #출력이 세가지가 가능하다는 것 1, -1, 0

        self.critic_model.compile(
            loss=[self.wasserstein, self.wasserstein, partial_gp_loss]  # Real, Fake, 보간 이미지에 대한 로스
    ...
    def save_model(self, run_folder):
        ...
        with open(run_folder + 'obj_d_losses.pkl', 'wb') as pkl_1:
            pickle.dump(self.d_losses, pkl_1)

        with open(run_folder + 'obj_g_losses.pkl', 'wb') as pkl_2:
            pickle.dump(self.g_losses, pkl_2)
